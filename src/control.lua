  ...
  if #connected_neighbours > 0 then
    ...
    while connected_neighbour do
      ...
      if enable_endpoint_check then
        if
          (isa_intake and string.find(connected_neighbour.entity.name, "outtake"))
          or (not isa_intake and string.find(connected_neighbour.entity.name, "intake"))
        then
          ducts = ducts - 1
        end
    ...
    if ducts > max_connections[1] or non_ducts > max_connections[2] then
      tooltip.showOnSurfaceGreyText(entity.surface, entity.position, {
      ...
      return true
    elseif
      enable_endpoint_check
      and ((isa_intake and connected_to_an_intake) or (not isa_intake and connected_to_an_outtake))
    then
...
function lookingforDuctJoin(entity)
  ...
  if join_pattern and join_pattern.successor then
    ...
    if joinable_candidate then
      ...
      local joinable_candidate_contained_fluid_name, joinable_candidate_contained_fluid_quantity = next(
        joinable_candidate.entity.get_fluid_contents()
      )
      ...
      if joinable_candidate_contained_fluid_quantity then
        entity.insert_fluid({
          name = joinable_candidate_contained_fluid_name,
          amount = joinable_candidate_contained_fluid_quantity,
        })
      end
...
function ductControlEvents(event)
  local entity = event.created_entity or false
  local removed = false
...
function ductScriptBuiltEvents(event)
